    ...
    static class AudioClipExtensions
    {
        public static async Task Play(this AudioClip audioClip, CancellationToken token, Action<float> timeUpdate = null, SoundEnvelopeSettings envelopeSettings = null)
        {
            ...
            try
            {
                ...
                while (true)
                {
                    var state = await EditorTask.RunOnMainThread(() => (isPlaying: audioSource.isPlaying, time: audioSource.time));
                    if (!state.isPlaying)
                        break;

                    await EditorTask.RunOnMainThread(() =>
                    {
                        try { timeUpdate?.Invoke(timeOffset + state.time); }
                        catch { /* ignored */ }
                    });
            ...
            {
                // Task.Delay throws on token cancellation; preserve non-throwing semantics.
            }
            finally
            {
                await EditorTask.RunOnMainThread(() =>
