...
namespace Unity.AI.Toolkit.Accounts.Components
{
    ...
    partial class Debugger : VisualElement
    {
        ...
        public Debugger()
        {
            ...
            // Only two of the three subscription states are reachable from a toggle. The unknown
            // state is what the backend reports when it could not determine the subscription, and
            // Clear covers it by leaving no settings at all.
            var hasAiSubscription = this.Q<Toggle>("has-ai-subscription");
            hasAiSubscription.RegisterValueChangedCallback(evt => Account.settings.Value = Account.settings.Value with {HasAiSubscription = evt.newValue});
            Account.settings.settings.Use(value =>
            {
                if (value == null)
                    return;
                hasAiSubscription.SetValueWithoutNotify(value.HasAiSubscription ?? false);
            });
