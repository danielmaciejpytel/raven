        ...
        protected virtual VisualElement CurrentView()
        {
            ...
            else if (!disableAIToolkitAccountCheck)
            {
                if (Account.settings.Value == null)
                    ...
                    current = m_AIDisabledPackageBanner ??= new();
                // Access first, then credits: credits are only actionable once the user can spend
                // them. Seat and subscription are reported together, so someone missing both is not
                // sent to fix one and left to discover the other on their next visit. Every case
                // routes to the dashboard credits page, which owns what can actually be done there.
