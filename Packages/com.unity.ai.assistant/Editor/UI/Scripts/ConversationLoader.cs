...
namespace Unity.AI.Assistant.UI.Editor.Scripts
{
    ...
    internal class ConversationLoader
    {
        ...
        /// <summary>
        /// Asks the Unity provider to refresh its conversations. The refresh honors the
        /// provider's cooldown, so a given call may be suppressed.
        /// </summary>
        void LoadUnityProviderConversations()
        {
            ...
            MainThread.DispatchAndForget(async () =>
            {
                try
                {
                    // A cooldown-suppressed call returns without firing
                    // OnUnityProviderConversationsRefreshed; the panel keeps its
                    // current Unity slice until a later refresh clears the cooldown.
                    await m_UnityProvider.RefreshConversationsAsync(enforceCooldown: true);
                }
                catch (Exception ex)
                {
