...
namespace Unity.AI.Assistant.Editor.Utils
{
    ...
    static class ProjectScriptCompilation
    {
        ...
        static readonly DomainReloadRequestGate k_DomainReloadRequestGate = new ();
        ...
        {
            if (!k_DomainReloadRequestGate.TryRequest())
            {
                InternalLog.Log("[ForceDomainReload] Domain reload already requested");
                return;
            }
            ...
            {
                k_DomainReloadRequestGate.Reset();
                throw;
            }
        }

        internal sealed class DomainReloadRequestGate
        {
            int m_IsRequested;

            public bool TryRequest() => Interlocked.Exchange(ref m_IsRequested, 1) == 0;

            public void Reset() => Interlocked.Exchange(ref m_IsRequested, 0);
        }
